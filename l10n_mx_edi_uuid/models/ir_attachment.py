...
class IrAttachment(models.Model):
    ...
    l10n_mx_edi_cfdi_uuid = fields.Char(
        string="Fiscal Folio", index=True, prefetch=False, readonly=True
    )
    ...
    def update_uuid(self):
        ...
        for attach in uuid_attachments:
            ...
            try:
                tree = (
                    model if model._name == "account.move" else model.move_id
                )._l10n_mx_edi_decode_cfdi(cfdi)
        ...
        ).write({"l10n_mx_edi_cfdi_uuid": False})
        invoice_ids = (
            (uuid_attachments - attachments_skipped)
