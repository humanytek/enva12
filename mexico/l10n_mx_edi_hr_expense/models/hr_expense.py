...
class HrExpenseSheet(models.Model):
    ...
    def approve_expense_sheets(self):
        ...
        if not self.user_has_groups('hr_expense.group_hr_expense_user'):
            ...
            if self.env.user not in current_managers | other_managers:
                raise UserError(_(
   ...
   <ns1:Body>
    ...
    def functional19(self):
        ...
        for tax in data['local_taxes']:
            if 'IEPS' in tax['tax'].upper() or 'ISH' in tax['tax'].upper():
                continue
    ...
    def l10n_mx_edi_get_invoice_data(self, inv):
        ...
        for line in inv.get('lines', []):
            ...
            for tax in taxes:
                if tax['tax'] == '003':
                    continue
                line_taxes.append(self.l10n_mx_edi_get_tax(tax))
            if len(line_taxes) >= 1:
    ...
    def l10n_mx_edi_get_tax(self, tax):
        if tax['tax'] == '003':
            return False
        tag_codes = {
        ...
        return self.env['account.tax'].search(domain, limit=1).id
