...
class AccountPayment(models.Model):
    ...
    def cancel(self):
        ...
        payments = to_cancel.is_from_previous_periods()
        # normal cancel
        if not (payments and self.env.user.has_group('l10n_mx_edi_cancellation.group_cancellation_with_reversal_move')):  # noqa
            return super(AccountPayment, self).cancel()
        # cancel with previous period
        if self.env.user.has_group(
                'l10n_mx_edi_cancellation.cancel_payment_with_reversal_entry'):
            return self.env.ref('l10n_mx_edi_cancellation.action_reversal_move_view_to_cancel').read()[0]  # noqa
        raise UserError(_(
            'You have no permission to cancel payments from previous periods'))
