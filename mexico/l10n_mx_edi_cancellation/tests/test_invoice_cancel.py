...
class TestL10nMxEdiCancelTest(InvoiceTransactionCase):
    ...
    def test_reset2draft(self):
        """Verify that invoice could be reset 2 draft"""
        invoice = self.create_invoice()
        invoice.sudo().journal_id.update_posted = True
        invoice.action_invoice_open()
        invoice.refresh()
        ...
        invoice.refresh()
        self.assertEquals(invoice.state, 'draft', 'The invoice cannot be '
                          'set in draft.')

    def test_no_reset2draft(self):
        """Verify that invoice is not reset 2 draft"""
        invoice = self.create_invoice()
        date = (self.env['l10n_mx_edi.certificate'].sudo().
                get_mx_current_datetime().date())
        invoice.date_invoice = date - relativedelta(months=1)
        invoice.sudo().journal_id.update_posted = True
        invoice.action_invoice_open()
        invoice.refresh()
        invoice.action_invoice_cancel()
        ...
        invoice.action_invoice_draft()
        invoice.refresh()
        self.assertEquals(invoice.state, 'cancel', 'The invoice was reset to '
                          'draft.')
