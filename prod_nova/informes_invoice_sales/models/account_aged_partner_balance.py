...
class report_account_aged_partner_sales(models.AbstractModel):
    _name = "account.aged.partner.sales"
    ...
    def _get_templates(self):
        templates = super(report_account_aged_partner_sales, self)._get_templates()
        templates['main_template'] = 'informes_invoice_sales.template_aged_partner_balance_report_sale'
        try:
            self.env['ir.ui.view'].get_view_id('informes_invoice_sales.template_aged_partner_balance_line_report_sale')
            templates['line_template'] = 'informes_invoice_sales.template_aged_partner_balance_line_report_sale'
        except ValueError:
            pass
        return templates
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.env['report.account.report_agedpartnerbalance'].with_context(include_nullified_amount=True)._get_partner_move_lines_sales(account_types, self._context['date_to'], 'posted', 30)
...
class report_account_aged_receivable_sales(models.AbstractModel):
    _name = "account.aged.receivable.sales"
    _description = "Aged Receivable"
    _inherit = "account.aged.partner.sales"

    def _set_context(self, options):
        ctx = super(report_account_aged_receivable_sales, self)._set_context(options)
    ...
    def _get_templates(self):
        templates = super(report_account_aged_receivable_sales, self)._get_templates()
        templates['line_template'] = 'informes_invoice_sales.line_template_aged_receivable_report_sale'
        return templates
...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines_sales(self, account_type, date_from, target_move, period_length):
        ...
        partner_clause += 'AND (l.partner_id NOT IN %s)'
        partner_notids = self.env['res.partner'].search(['|',('name', 'ilike', 'D-'),('name', 'ilike', 'A-')]).ids
        arg_list += (tuple(partner_notids),)
        partner_clause += 'AND (account_account.id NOT IN %s)'
        account_accound_ids = self.env['account.account'].search([('code', 'ilike', '107.01.001')]).ids
        arg_list += (tuple(account_accound_ids),)
        arg_list += (date_from, tuple(company_ids))

        query = '''
            SELECT DISTINCT l.partner_id, UPPER(res_partner.name)
            FROM account_move_line AS l
            left join res_partner on l.partner_id = res_partner.id, account_account, account_move am
