    ...
    def _partner_trend(self,options,line_id):
        ...
        sql_query ="""
            (SELECT
                    ...
                    LEFT JOIN account_invoice ai ON ai.id=ail.invoice_id
                    LEFT JOIN res_partner rp ON rp.id=ail.partner_id
                    LEFT JOIM trend_budget_sales tbs ON tbs.name=rp.id
                    WHERE ai.state!='draft' AND ai.state!='cancel' AND ai.type='out_invoice' AND ai.date_applied >= '"""+date_from+"""' AND ai.date_applied <= '"""+date_to+"""'
                    AND ai.user_id not in (90) AND pt.name not ilike 'ANTICIPO DE CLIENTE%' AND pt.name not ilike 'TRANSPORTACION%' AND pt.name not ilike 'CHATARRA%' AND pt.name not ilike 'PUB GRAL VTA CHATARRA%'
            ...
            for invoice in invoices:
                ...
                lines.append({
                        ...
                        'columns':[
                            ...
                            {'name':"{:,.2f}".format(round(invoices_line[2]/1000))},
                            ...
                            {'name':"{:.2%}".format(0) if budget==0 else "{:.2%}".format(round((invoices_line[2]/1000)/(budget/1000)))},
                            # {'name':"{:.2%}".format(0) if budget==0 else "{:.2%}".format(((invoices_line[2]/1000)-(budget/1000))/(budget/1000))},
                            {'name':"{:.2%}".format(0) if budget==0 else "{:.2%}".format(round(((invoices_line[2]/1000)/(((budget/1000)/bussines_days.bussines_days)*self._billed_days(options,line_id))-1)))},
                            {'name':"{:.2%}".format(round(desv_price_per_kg)) },
                            ...
                            {'name':0 if invoices_line_promedio[2]==0 else "{:,.2f}".format(round((invoices_line_promedio[2]/12)/1000)) },
                            {'name':"{:,.2f}".format(round((invoices_line_lymonth[2])/1000)) },
