...
class ReportIndicartors(models.AbstractModel):
    ...
    def _get_columns(self, options):
        ...
        header2 = [
            ...
            {'name': '','class': 'number', 'style': 'white-space:nowrap;', 'colspan': 1},
            {'name': '','class': 'number', 'style': 'white-space:nowrap;', 'colspan': 1},
        ]
    ...
    def _get_total_indicators(self, options,date_to,date_from, campo):
        ...
        if campo == 'tons_lam_real':
            ...
            campo = """coalesce(sum(coalesce(""" + str(campo)+""",0)),0) as """ + str(campo)+""

        sql_query = """select """ + \
            str(campo) + """ from indicators_nova ia where ia.indicator_date >= '""" + \
            str(date_to)+"""' and ia.indicator_date <= '""" +  str(date_from)+"""'"""
    ...
    def _get_lines(self, options, line_id=None):
        ...
        subheading = [{'header': 'CORRUGADORA FOSBER', 'subheader': 'Toneladas', 'campo': 'ton_fosber', 'bandera': 0}, {'header': 'CORRUGADORA FOSBER', 'subheader': 'Mezcla Promedio', 'campo': 'mezcla_prom', 'bandera': 0},
            ...
            {'header': 'PAGOS', 'subheader': 'Cartera', 'campo': 'cartera', 'bandera': 1}, {
                          'header': 'PAGOS', 'subheader': 'Proyecto', 'campo': 'project', 'bandera': 0},
        ...
        for h in header:
            if (h['cantidad'] == 'si'):
                ...
                if (h['bandera'] == 0):
                    indicator = self._get_indicators_line(options,df, str(h['campo']))
                    indicator2 = self._get_indicators_line(options,df2, str(h['campo']))
                    indicator3 = self._get_indicators_line(options,df3, str(h['campo']))
                    indicator4 = self._get_indicators_line(options,df4, str(h['campo']))
                    indicator5 = self._get_indicators_line(options,df5, str(h['campo']))
                    indicator6 = self._get_indicators_line(options,df6, str(h['campo']))
                    indicator7 = self._get_indicators_line(options,df7, str(h['campo']))
                    total_month = self._get_total_indicators(options,new_date_from, df,str(h['campo']))
                    totalsemana = (indicator[0][h['campo']] if indicator else 0) + (indicator2[0][h['campo']] if indicator2 else 0) + (indicator3[0][h['campo']] if indicator3 else 0) + (indicator4[0][h['campo']] if indicator4 else 0) + (indicator5[0][h['campo']] if indicator5 else 0) + (indicator6[0][h['campo']] if indicator6 else 0) + (indicator7[0][h['campo']] if indicator7 else 0)
                    ...
                    indicator.append({'saldo': (dolar[0]['dolar'] if dolar else 0) + (euro[0]['euro'] if euro else 0) + (mxn[0]['mxn'] if mxn else 0)})
                    ...
                    indicator2.append({'saldo': (dolar2[0]['dolar'] if dolar2 else 0) + (euro2[0]['euro'] if euro2 else 0) + (mxn2[0]['mxn'] if mxn2 else 0)})
                    ...
                    indicator3.append({'saldo': (dolar3[0]['dolar'] if dolar3 else 0) + (euro3[0]['euro'] if euro3 else 0) + (mxn3[0]['mxn'] if mxn3 else 0)})
                    ...
                    indicator4.append({'saldo': (dolar4[0]['dolar'] if dolar4 else 0) + (euro4[0]['euro'] if euro4 else 0) + (mxn4[0]['mxn'] if mxn4 else 0)})
                    ...
                    indicator5.append({'saldo': (dolar5[0]['dolar'] if dolar2 else 0) + (euro5[0]['euro'] if euro5 else 0) + (mxn5[0]['mxn'] if mxn5 else 0)})
                    ...
                    indicator6.append({'saldo': (dolar6[0]['dolar'] if dolar6 else 0) + (euro6[0]['euro'] if euro6 else 0) + (mxn6[0]['mxn'] if mxn6 else 0)})
                    ...
                    indicator7.append({'saldo': (dolar7[0]['dolar'] if dolar7 else 0) + (euro7[0]['euro'] if euro7 else 0) + (mxn7[0]['mxn'] if mxn7 else 0)})
                 ...
                 #   total_month = self._get_total_query(options,new_date_from, df,str(h['campo']))
                    ...
                    while (iterator < (diferencia.days + 1)):
                         dolar = self._get_lines_query(options,fields.Date.from_string(date_initial.date())+timedelta(days=iterator), str('dolar'))
                         euro = self._get_lines_query(options,fields.Date.from_string(date_initial.date())+timedelta(days=iterator), str('euro'))
                         mxn = self._get_lines_query(options,fields.Date.from_string(date_initial.date())+timedelta(days=iterator), str('mxn'))
                         total = total + (dolar[0]['dolar'] if dolar else 0) + (euro[0]['euro'] if euro else 0) + (mxn[0]['mxn'] if mxn else 0)
                         iterator = iterator + 1
                    
                    total_month = total / iterator
                    totalsemana = ((indicator[0][h['campo']] if indicator else 0) + (indicator2[0][h['campo']] if indicator2 else 0) + (indicator3[0][h['campo']] if indicator3 else 0) + (indicator4[0][h['campo']] if indicator4 else 0) + (indicator5[0][h['campo']] if indicator5 else 0) + (indicator6[0][h['campo']] if indicator6 else 0) + (indicator7[0][h['campo']] if indicator7 else 0))/7
                ...
                for s in subheading:
                    if (s['header'] == h['name']):
                        if (s['bandera'] == 0):
                            indicator = self._get_indicators_line(options, df, str(s['campo']))
                            indicator2 = self._get_indicators_line(options,df2, str(s['campo']))
                            indicator3 = self._get_indicators_line(options,df3, str(s['campo']))
                            indicator4 = self._get_indicators_line(options,df4, str(s['campo']))
                            indicator5 = self._get_indicators_line(options,df5, str(s['campo']))
                            indicator6 = self._get_indicators_line(options,df6, str(s['campo']))
                            indicator7 = self._get_indicators_line(options,df7, str(s['campo']))
                            total_month = self._get_total_indicators(options,new_date_from, df,str(s['campo']))
                 
                            
                            totalsemana = (indicator[0][s['campo']] if indicator else 0) + (indicator2[0][s['campo']] if indicator2 else 0) + (indicator3[0][s['campo']] if indicator3 else 0) + (indicator4[0][s['campo']] if indicator4 else 0) + (indicator5[0][s['campo']] if indicator5 else 0) + (indicator6[0][s['campo']] if indicator6 else 0) + (indicator7[0][s['campo']] if indicator7 else 0)
                           ...
                           {'name': "{:,.3f}".format(
                            ...
                            if s['campo'] == 'saldo':
                                ...
                                if s['campo'] == 'total_cobranza':
                                    carton = self._get_lines_query(options, df,'venta_carton')
                                    papel = self._get_lines_query(options, df, 'venta_papel')
                                    ingresos = self._get_indicators_line(options, df, 'other_receipts')
                                    indicator.append({'total_cobranza':(carton[0]['venta_carton'] if carton else 0) + (papel[0]['venta_papel'] if papel else 0) + (ingresos[0]['other_receipts'] if ingresos else 0)})
                                    
                                    carton2 = self._get_lines_query(options, df2, 'venta_carton')
                                    papel2 = self._get_lines_query(options, df2, 'venta_papel')
                                    ingresos2 = self._get_indicators_line(options, df2, 'other_receipts')
                                    indicator2.append({'total_cobranza':(carton2[0]['venta_carton'] if carton2 else 0) + (papel2[0]['venta_papel'] if papel2 else 0) + (ingresos2[0]['other_receipts'] if ingresos2 else 0)})
                                    
                                    carton3 = self._get_lines_query(options, df3, 'venta_carton')
                                    papel3 = self._get_lines_query(options, df3, 'venta_papel')
                                    ingresos3 = self._get_indicators_line(options, df3, 'other_receipts')
                                    indicator3.append({'total_cobranza':(carton3[0]['venta_carton'] if carton3 else 0) + (papel3[0]['venta_papel'] if papel3 else 0) + (ingresos3[0]['other_receipts'] if ingresos3 else 0)})
                                    
                                    carton4 = self._get_lines_query(options, df4, 'venta_carton')
                                    papel4 = self._get_lines_query(options, df4, 'venta_papel')
                                    ingresos4 = self._get_indicators_line(options, df4,'other_receipts')
                                    indicator4.append({'total_cobranza':(carton4[0]['venta_carton'] if carton4 else 0) + (papel4[0]['venta_papel'] if papel4 else 0) + (ingresos4[0]['other_receipts'] if ingresos4 else 0)})
                                    
                                    carton5 = self._get_lines_query(options, df5, 'venta_carton')
                                    papel5 = self._get_lines_query(options, df5, 'venta_papel')
                                    ingresos5 = self._get_indicators_line(options, df5, 'other_receipts')
                                    indicator5.append({'total_cobranza':(carton5[0]['venta_carton'] if carton5 else 0) + (papel5[0]['venta_papel'] if papel5 else 0) + (ingresos5[0]['other_receipts'] if ingresos5 else 0)})
                                    
                                    carton6 = self._get_lines_query(options, df6, 'venta_carton')
                                    papel6 = self._get_lines_query(options, df6, 'venta_papel')
                                    ingresos6 = self._get_indicators_line(options, df6, 'other_receipts')
                                    indicator6.append({'total_cobranza':(carton6[0]['venta_carton'] if carton6 else 0) + (papel6[0]['venta_papel'] if papel6 else 0) + (ingresos6[0]['other_receipts'] if ingresos6 else 0)})
                                    
                                    carton7 = self._get_lines_query(options, df7, 'venta_carton')
                                    papel7 = self._get_lines_query(options, df7, 'venta_papel')
                                    ingresos7 = self._get_indicators_line(options, df7, 'other_receipts')
                                    indicator7.append({'total_cobranza':(carton7[0]['venta_carton'] if carton7 else 0) + (papel7[0]['venta_papel'] if papel7 else 0) + (ingresos7[0]['other_receipts'] if ingresos7 else 0)})
                                    
                                    carton8 = self._get_total_query(options,new_date_from, df, 'venta_carton')
                                    papel8 = self._get_total_query(options,new_date_from, df, 'venta_papel')
                                    ingresos8 = self._get_total_indicators(options,new_date_from, df, 'other_receipts')
                                    total_month.append({'total_cobranza':(carton8[0]['venta_carton'] if carton8 else 0) + (papel8[0]['venta_papel'] if papel8 else 0) + (ingresos8[0]['other_receipts'] if ingresos8 else 0)})
                                    
                                elif s['campo'] == 'total_pagos':
                                    contado = self._get_lines_query(options, df, 'contado')
                                    cartera = self._get_lines_query(options, df, 'cartera')
                                    proyecto = self._get_indicators_line(options, df, 'project')
                                    indicator.append({'total_pagos': (contado[0]['contado'] if contado else 0) + (cartera[0]['cartera'] if cartera else 0) + (proyecto[0]['project'] if proyecto else 0)})
                                    
                                    contado2 = self._get_lines_query(options, df2, 'contado')
                                    cartera2 = self._get_lines_query(options, df2, 'cartera')
                                    proyecto2 = self._get_indicators_line(options, df2, 'project')
                                    indicator2.append({'total_pagos': (contado2[0]['contado'] if contado2 else 0) + (cartera2[0]['cartera'] if cartera2 else 0) + (proyecto2[0]['project'] if proyecto2 else 0)})
                                    
                                    contado3 = self._get_lines_query(options, df3, 'contado')
                                    cartera3 = self._get_lines_query(options, df3, 'cartera')
                                    proyecto3 = self._get_indicators_line(options, df3, 'project')
                                    indicator3.append({'total_pagos': (contado3[0]['contado'] if contado3 else 0) + (cartera3[0]['cartera'] if cartera3 else 0) + (proyecto3[0]['project'] if proyecto3 else 0)})
                                    
                                    contado4 = self._get_lines_query(options, df4, 'contado')
                                    cartera4 = self._get_lines_query(options, df4, 'cartera')
                                    proyecto4 = self._get_indicators_line(options, df4, 'project')
                                    indicator4.append({'total_pagos': (contado4[0]['contado'] if contado4 else 0) + (cartera4[0]['cartera'] if cartera4 else 0) + (proyecto4[0]['project'] if proyecto4 else 0)})
                                    
                                    contado5 = self._get_lines_query(options, df5, 'contado')
                                    cartera5 = self._get_lines_query(options, df5, 'cartera')
                                    proyecto5 = self._get_indicators_line(options, df5, 'project')
                                    indicator5.append({'total_pagos': (contado5[0]['contado'] if contado5 else 0) + (cartera5[0]['cartera'] if cartera5 else 0) + (proyecto5[0]['project'] if proyecto5 else 0)})
                                    
                                    contado6 = self._get_lines_query(options, df6, 'contado')
                                    cartera6 = self._get_lines_query(options, df6, 'cartera')
                                    proyecto6 = self._get_indicators_line(options, df6, 'project')
                                    indicator6.append({'total_pagos': (contado6[0]['contado'] if contado6 else 0) + (cartera6[0]['cartera'] if cartera6 else 0) + (proyecto6[0]['project'] if proyecto6 else 0)})
                                    
                                    contado7 = self._get_lines_query(options, df7, 'contado')
                                    cartera7 = self._get_lines_query(options, df7, 'cartera')
                                    proyecto7= self._get_indicators_line(options, df7, 'project')
                                    indicator7.append({'total_pagos': (contado7[0]['contado'] if contado7 else 0) + (cartera7[0]['cartera'] if cartera7 else 0) + (proyecto7[0]['project'] if proyecto7 else 0)})
                                    
                                    contado8 = self._get_total_query(options,new_date_from, df, 'contado')
                                    cartera8 = self._get_total_query(options,new_date_from, df,'cartera')
                                    proyecto8= self._get_total_indicators(options,new_date_from, df, 'project')
                                    total_month.append({'total_pagos': (contado8[0]['contado'] if contado8 else 0) + (cartera8[0]['cartera'] if cartera8 else 0) + (proyecto8[0]['project'] if proyecto8 else 0)})
                                
                                else:
                                    indicator = self._get_lines_query(options, df, str(s['campo']))
                                    indicator2 = self._get_lines_query(options,df2, str(s['campo']))
                                    indicator3 = self._get_lines_query(options,df3, str(s['campo']))
                                    indicator4 = self._get_lines_query(options,df4, str(s['campo']))
                                    indicator5 = self._get_lines_query(options,df5, str(s['campo']))
                                    indicator6 = self._get_lines_query(options,df6, str(s['campo']))
                                    indicator7 = self._get_lines_query(options,df7, str(s['campo']))
                                    total_month = self._get_total_query(options,new_date_from, df,str(s['campo']))
                                    
                                    
                                
                                if s['campo'] == 'kg_carton' or s['campo'] == 'kg_papel' or s['campo'] == 'kg_puebla':
                                    promedio = self._get_total_query(options,df7, df,str(s['campo']))
                                    totalsemana = promedio[0][s['campo']] if promedio or promedio[0][s['campo']] is None  else 0
                                elif s['campo'] == 'price_lam':
                                    promedio = self._get_total_indicators(options,df7, df,str(s['campo']))
                                    totalsemana = promedio[0][s['campo']] if promedio else 0
                                else:
                                    totalsemana = (indicator[0][s['campo']] if indicator else 0) + (indicator2[0][s['campo']] if indicator2 else 0) + (indicator3[0][s['campo']] if indicator3 else 0) + (indicator4[0][s['campo']] if indicator4 else 0) + (indicator5[0][s['campo']] if indicator5 else 0) + (indicator6[0][s['campo']] if indicator6 else 0) + (indicator7[0][s['campo']] if indicator7 else 0)       
