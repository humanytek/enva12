...
class hr_applicant_nova(models.Model):
    ...
    ref_job = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Referencias Laborales',
    tracking=True,
    )

    psiconometric = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Psiconometría',
    tracking=True,
    )

    alcoholimetria = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Alcholimetria',
    tracking=True,
    )

    antidoping = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Antidoping',
    tracking=True,
    )

    p_confianza = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Prueba de Confianza',
    tracking=True,
    )

    test_driver = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Prueba de Manejo',
    tracking=True,
    )

    v_psicologic = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Valoracion Psicológica',
    tracking=True,
    )

    v_medica = fields.Selection(
    [('realizado', 'Realizado'), ('no_aplica', 'No aplica')],
    string='Valoracion Médica',
    tracking=True,
    )
